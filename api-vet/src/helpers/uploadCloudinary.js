...
const subirImagenCloudinary = async (filePath, folder = "Pacientes") => {
    ...
    const { secure_url, public_id } = await cloudinary.uploader.upload(filePath, { folder })   // Recibe la ruta y la carpeta 
    // una vez que se sube la  imagen, elimino el archivo temporal que se creó en el servidor
